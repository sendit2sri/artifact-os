...
import { createProject, fetchWorkspaces, fetchWorkspaceProjects } from "@/lib/api";
import { Z } from "@/lib/tokens";
import { useCurrentWorkspace } from "@/hooks/useCurrentWorkspace";
import { WorkspaceSelector } from "@/components/WorkspaceSelector";
import { Loader2, Sparkles, ShieldCheck, FileUp, Zap, Plus, FolderOpen } from "lucide-react";
...
export default function HomePage() {
  ...
  const { data: projects = [], isLoading: projectsLoading } = useQuery({
    ...
    queryFn: () => fetchWorkspaceProjects(currentWorkspaceId!),
  ...
  const handleWorkspaceSelect = (workspaceId: string) => setCurrentWorkspaceId(workspaceId);
  ...
  const handleStart = async () => {
    ...
    try {
      setLoading(true);
      const project = await createProject(wsId, "New Research Project");
  ...
  const handleNewProject = async () => {
    ...
    try {
      setLoading(true);
      const project = await createProject(wsId, "New Research Project");
      router.push(`/project/${project.id}?ws=${encodeURIComponent(wsId)}`);
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF9] dark:bg-[#1A1A1A] transition-colors">
      <header className={`h-16 flex items-center justify-between px-6 lg:px-8 border-b border-stone-200 dark:border-stone-800 bg-white/80 dark:bg-[#242424]/80 backdrop-blur-sm relative ${Z.header}`}>
      ...
      <main className="max-w-4xl mx-auto px-6 lg:px-8 pt-20 pb-16">
        <div className="text-center space-y-6">
          ...
          <div className="max-w-2xl mx-auto pt-4">
            <div className="flex flex-col sm:flex-row gap-3 p-3 bg-white dark:bg-[#2A2A2A] rounded-full shadow-lg border border-stone-200 dark:border-stone-700">
              ...
              >
                {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Starting...</> : <><Sparkles className="h-4 w-4 mr-2" />Start Research</>}
              </Button>
            </div>
            <p className="text-xs text-stone-500 dark:text-stone-400 mt-3">Support for URLs, PDFs, and Markdown files (Press Enter to start)</p>
          </div>
          <div className="flex flex-wrap justify-center gap-3 pt-8">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-[#2A2A2A] border border-stone-200 dark:border-stone-700 text-sm text-stone-700 dark:text-stone-300 shadow-sm">
              <ShieldCheck className="h-4 w-4 text-emerald-600 dark:text-emerald-500" /> Citation verification
            </div>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-[#2A2A2A] border border-stone-200 dark:border-stone-700 text-sm text-stone-700 dark:text-stone-300 shadow-sm">
              <Zap className="h-4 w-4 text-amber-600 dark:text-amber-500" /> AI-powered synthesis
            </div>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-[#2A2A2A] border border-stone-200 dark:border-stone-700 text-sm text-stone-700 dark:text-stone-300 shadow-sm">
              <FileUp className="h-4 w-4 text-blue-600 dark:text-blue-500" /> Multi-format support
            </div>
          </div>
        </div>
        ...
        <div className="mt-24">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-[#1F1F1F] dark:text-stone-100 tracking-tight">Projects</h2>
            <Button
              size="sm"
              variant="outline"
          ...
          {!currentWorkspaceId ? (
            <div className="rounded-2xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-[#2A2A2A] p-12 text-center">
              ...
              <p className="text-stone-600 dark:text-stone-300 font-medium mt-4">Loading workspaces...</p>
            </div>
          ) : projectsLoading ? (
            <div className="rounded-2xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-[#2A2A2A] p-12 text-center">
              <Loader2 className="h-8 w-8 animate-spin mx-auto text-stone-400" />
              <p className="text-stone-600 dark:text-stone-300 font-medium mt-4">Loading projects...</p>
            </div>
          ) : projects.length === 0 ? (
            <div className="rounded-2xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-[#2A2A2A] p-12 text-center">
              ...
              <p className="text-sm text-stone-500 dark:text-stone-400 max-w-md mx-auto mb-4">
                Create your first research project to start extracting and organizing insights from your sources.
              </p>
              <Button onClick={handleNewProject} disabled={loading} data-testid="project-create">
            ...
            <ul className="grid gap-3" data-testid="projects-list">
              {projects.map((p) => (
                <li key={p.id}>
                  <a
                    href={`/project/${p.id}?ws=${encodeURIComponent(p.workspace_id)}`}
                    className="flex items-center gap-3 p-4 rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-[#2A2A2A] hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors"
                    data-testid="project-card"
                  >
                    <FolderOpen className="h-5 w-5 text-stone-400 shrink-0" />
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-[#1F1F1F] dark:text-stone-100 truncate">{p.title}</p>
                      <p className="text-xs text-stone-500 dark:text-stone-400">Project</p>
                    </div>
                  </a>
                </li>
              ))}
